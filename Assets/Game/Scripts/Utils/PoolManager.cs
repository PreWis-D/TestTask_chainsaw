...
public class PoolManager
{
    ...
    #region GetGenerics
        ...
        public static T GetPool<T>(T prefab, Vector3 pos, Quaternion quaternion) where T : Component
        {
        ...
        public static T GetPool<T>(T prefab, Vector3 pos) where T : Component
        {
            string name = prefab.gameObject.name;
            if (!_pools.ContainsKey(name))
                _pools[name] = new LinkedList<object>();
        ...
        private static T GetPoolFirst<T>(string name, Vector3 pos, Quaternion quaternion) where T : Component
        {
    ...
    public static void PoolPreLaunch<T>(T prefab, int count, Transform container) where T : Component
    {
        string name = prefab.gameObject.name;
        if (!_pools.ContainsKey(name))
            _pools[name] = new LinkedList<object>();
    ...
    public static void SetPool<T>(T target) where T: Component
    {
    ...
    /// <summary>
    /// It is necessary to clear the pool if you changed the game scene, since the pool can store links to inactive objects
    /// </summary>
